...
module.exports = async function (client, message) {
    ...
    if (ratelimit) return author.send(embed.setDescription(`You are being ratelimited! Please wait for **${humanizeDuration(ratelimit - Date.now())}**`).catch(() => { return }))
    ...
    if (guild) {
        if (!guild.database) {
            ...
            guild.database = await new MongoGuild(guild.id).get()
        }
        ...
        if (permissions) {
            ...
            if (missingPermissions.length) return channel.send(member, embed.setDescription(`**You are missing the following permissions:** ${missingPermissions.join(', ')}`))
        }
